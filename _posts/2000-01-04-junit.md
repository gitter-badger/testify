...
implementation that configures, verifies, wires, and runs the test class.

The next thing you will notice are two annotations, `@Cut` and `@Fake`. The
`@Cut`annotation denotes the class under test and `@Fake` denotes the desire to
...
tests run in isolation and you do not have to worry about managing state between
test runs. It is all done for you so you can focus on writing your test cases
not boilerplate code to manage test state.
...
verifies, wires, and runs the test class.
...
loading a module that defines services we are want to test and working with
real objects rather than fake ones.
...
We let the Testify know this by annotating the test class with
...
when you do not want to use the real instance of the class under test's
collaborators (i.e. credit card order), and in those instances you can use
`@Fake` to work with fake instance of the `Greeting` class.
...
Well, Testify inspects the test class fields and the constructor of the
class under test, it then:
...
### Example Spring Integration Test
Given a the following Spring MVC Application:
...
Once again you will note that we are running with `SpringSystemTest`. It is
similar to `SpringJunitTest` and `SpringIntegrationTest` in that it configures,
verifies, wires, and runs the test class.

Since this is an system test we want to verify the application works from the
client perspective. This means we start the Spring application using an a servlet
container. By default, Testify uses Undertow servlet container but you have
the option of using whatever servlet container you preferred. We will cover this
functionality in the advanced topic section.
...
that is aware of the base URI the server is running on and injects it into the
test class. By default Testify uses JerseyClient for HTTP/REST communication.
Again, the underlying client implementation is configurable.

Once the server is running and the client instance is configured and injected
into the test class you can start testing the `GreeterResource` by using
the client instance.

If you are curious about what is going on behind the scenes, it is pretty
straight forward, Testify:

1. creates a proxy `GreetingApplication`
1. creates a new Undertow servlet container instance and deploy the proxy application
1. gets a reference to the runing applicaiton's Spring applicaiton context
1. creates and configures a `ClientInstace` instance aware of the server base URI and
adds it to to the running application's Spring application context
...
before each test and insures the server and client are shutdown/closed properly.
