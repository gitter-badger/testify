...
On Macs and Windows Docker installation and configuration process is a bit
different as they require a VM. Follow the Docker documentation to
[install](https://docs.docker.com/machine/install-machine/) and
[configure](https://docs.docker.com/machine/get-started/) Docker-Machine and
then follow [Docker Environment configuration](https://docs.docker.com/machine/reference/env)
documentation to configure your Docker-Machine environment.

#### Notes on Docker Configuration

By default Testify communicates with the Docker Remote API through non-secure
`http://127.0.0.1:2375` URL endpoint. If you wish to expose an endpoint
with a different IP address and port or are using a Docker-Machine, or want to
use secure-communication then you will need to explictly configure Docker Client
used by Testify in your test classes:
{% highlight java linenos=table %}
@Config
